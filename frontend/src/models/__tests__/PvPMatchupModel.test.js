...
describe('PvPMatchupModel', () => {
    ...
    it('throws if model is disabled by config', async () => {
        ...
        await expect(getPvPMatchupFeatures('a', 'b', 'nba', {})).rejects.toThrow('PvPMatchupModel is disabled by config.');
    });
    it('throws for unsupported sport', async () => {
        await expect(getPvPMatchupFeatures('a', 'b', 'cricket', {})).rejects.toThrow('Unsupported sport type: cricket');
    });
});
