...
test.describe('Prediction Flow', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await page.route('**/api/risk-profile', async (route) => {
            await route.fulfill({
                status: 200,
    ...
    test('displays prediction recommendations', async ({ page }) => {
    ...
    test('filters recommendations by risk level', async ({ page }) => {
        // Wait for recommendations to load
        await page.waitForSelector('[data-testid="bet-recommendation-card"]');
        ...
        for (const card of cards) {
            const riskLevel = await card.$eval('[data-testid="risk-level"]', (el) => el.textContent);
            expect(riskLevel).toBe('LOW');
        }
    });
    test('sorts recommendations by confidence', async ({ page }) => {
        // Wait for recommendations to load
        await page.waitForSelector('[data-testid="bet-recommendation-card"]');
        // Select confidence sort
        await page.selectOption('[data-testid="sort-select"]', 'confidence');
        // Verify sorted recommendations
        const confidences = await page.$$eval('[data-testid="confidence-value"]', (elements) => elements.map(el => parseFloat(el.textContent || '0')));
    ...
    test('displays prediction explanation modal', async ({ page }) => {
        // Wait for recommendations to load
        await page.waitForSelector('[data-testid="bet-recommendation-card"]');
        ...
        await expect(page.locator('[data-testid="modal-title"]')).toContainText('Prediction Explanation');
    ...
    test('switches between model explanations in modal', async ({ page }) => {
        // Open modal
        await page.waitForSelector('[data-testid="bet-recommendation-card"]');
        await page.click('[data-testid="view-details-button"]');
        await page.waitForSelector('[data-testid="prediction-explanation-modal"]');
    ...
    test('handles API errors gracefully', async ({ page }) => {
        ...
        await expect(page.locator('[data-testid="error-message"]')).toContainText('Failed to load recommendations');
    });
    test('updates recommendations when risk profile changes', async ({ page }) => {
        ...
        const stakes = await page.$$eval('[data-testid="stake-amount"]', (elements) => elements.map(el => parseFloat(el.textContent?.replace(/[^0-9.-]+/g, '') || '0')));
