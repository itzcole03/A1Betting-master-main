...
export class UnifiedWebSocketService extends BaseService {
    ...
    async connect() {
        try {
            ...
            if (!this.url ||
                ...
                this.logger.warn("WebSocket connection skipped: No valid WebSocket URL configured. Set VITE_WS_URL environment variable to enable WebSocket functionality.", this.name);
                return;
            }
    ...
    handleDisconnect() {
        this.stopPingInterval();
        this.clearPongTimeout();
        if (this.wsConfig.autoReconnect &&
            this.reconnectAttempts < this.wsConfig.reconnectAttempts) {
            this.reconnectAttempts++;
            const delay = Math.min(this.wsConfig.reconnectInterval *
                Math.pow(2, this.reconnectAttempts - 1), 30000);
            setTimeout(() => this.connect(), delay);
            this.logger.warn(`Attempting to reconnect (${this.reconnectAttempts}/${this.wsConfig.reconnectAttempts})`, this.name);
        }
        else {
