...
describe('BettingOpportunityService', () => {
    let service;
    let mockArbitrageService;
    let mockLineShoppingService;
    let mockPredictionService;
    let mockAdvancedPredictionService;
    let mockNotificationManager;
    beforeEach(() => {
        jest.clearAllMocks();
        service = BettingOpportunityService.getInstance();
        mockArbitrageService = ArbitrageService.getInstance();
        mockLineShoppingService = new LineShoppingService();
        mockPredictionService = PredictionService.getInstance();
        mockAdvancedPredictionService =
            AdvancedPredictionService.getInstance();
        mockNotificationManager = new NotificationManager();
    });
    describe('startMonitoring', () => {
        it('should start monitoring and notify', () => {
            service.startMonitoring();
            expect(mockNotificationManager.notifySystemAlert).toHaveBeenCalledWith('Monitoring Started', 'Betting opportunity monitoring has been activated', 'low');
    ...
    describe('stopMonitoring', () => {
        it('should stop monitoring and notify', () => {
            ...
            expect(mockNotificationManager.notifySystemAlert).toHaveBeenCalledWith('Monitoring Stopped', 'Betting opportunity monitoring has been deactivated', 'low');
    ...
    describe('handleArbitrageOpportunity', () => {
        it('should notify and emit when monitoring is active', () => {
            ...
            expect(mockNotificationManager.notifyArbitrageOpportunity).toHaveBeenCalledWith(mockOpportunity);
        });
    ...
    describe('handlePrediction', () => {
        it('should notify and emit when monitoring is active', () => {
            ...
            expect(mockNotificationManager.notifyModelUpdate).toHaveBeenCalledWith(`New prediction for ${mockPrediction.propId}`, `Confidence: ${(mockPrediction.confidence * 100).toFixed(1)}%`);
        });
        it('should not notify or emit when monitoring is inactive', () => {
            const mockPrediction = {
                propId: 'prop1',
    ...
    describe('handleAdvancedPrediction', () => {
        it('should notify and emit when monitoring is active', () => {
            ...
            expect(mockNotificationManager.notifyModelUpdate).toHaveBeenCalledWith('New Advanced Prediction', expectedMessage);
        });
        it('should not notify or emit when monitoring is inactive', () => {
            const mockAdvancedPrediction = {
                basePrediction: {
