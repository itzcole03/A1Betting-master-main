...
export class BettingOpportunityService extends EventEmitter {
    constructor() {
        ...
        this.VOLUME_WINDOW = 24 * 60 * 60 * 1000; // 24 hours in milliseconds
        ...
        this.advancedPredictionService.on('newAdvancedPrediction', this.handleAdvancedPrediction.bind(this));
    ...
    startMonitoring() {
        ...
        this.notificationManager.notifySystemAlert('Monitoring Started', 'Betting opportunity monitoring has been activated', 'low');
    }
    /**
    ...
    stopMonitoring() {
        ...
        this.isMonitoring = false;
        this.notificationManager.notifySystemAlert('Monitoring Stopped', 'Betting opportunity monitoring has been deactivated', 'low');
    }
    /**
    ...
    calculateVolatility(volumeHistory) {
    ...
    handleOddsUpdate(data) {
        ...
        this.arbitrageService.monitorOpportunities(new Map([[data.bookmakerId, new Map([[data.odds[0].market, data.odds]])]]));
    ...
    handlePrediction(prediction) {
        ...
        this.notificationManager.notifyModelUpdate(`New prediction for ${prediction.propId}`, `Confidence: ${(prediction.confidence * 100).toFixed(1)}%`);
        this.emit('prediction', prediction);
    }
    handleAdvancedPrediction(prediction) {
        if (!this.isMonitoring) {
            return;
        }
        const message = [
    ...
    handleMarketAnomaly(data) {
        if (!this.isMonitoring)
            return;
        data.anomalies.forEach(anomaly => {
            this.notificationManager.notifySystemAlert('Market Anomaly Detected', `${anomaly.type.toUpperCase()} anomaly: ${anomaly.description}`, anomaly.severity);
        });
        this.emit('marketAnomaly', data);
    }
    handleMarketEfficiency(data) {
        if (!this.isMonitoring)
            return;
        // Only notify for significant efficiency changes
        if (data.metrics.spreadEfficiency < 0.8) {
            this.notificationManager.notifySystemAlert('Low Market Efficiency', `Spread efficiency is below 80% for event ${data.eventId}`, 'medium');
        }
    ...
    cleanup() {
        ...
        for (const [eventId, metrics] of this.marketMetrics.entries()) {
            ...
            metrics.volume.volumeHistory = metrics.volume.volumeHistory.filter(v => now - v.timestamp < this.VOLUME_WINDOW);
