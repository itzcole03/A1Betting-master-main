...
const PropCard: React.FC<PropCardProps key={387456}> = ({ prop, sentiment, onViewDetails, className, team, position, statType, line, pickType = 'normal', trendValue, gameInfo, playerImageUrl }) => {
  ...
  return (
    <div; 
      ...
      onClick={() = key={506179}> onViewDetails(prop.id)}
    ...
    >
      <div className="flex items-center justify-between mb-2" key={120997}>
        <div className="flex items-center gap-2" key={100294}>
          {playerImageUrl && (
            <img src={playerImageUrl} alt={prop.player_name} className="w-10 h-10 rounded-lg object-cover border border-gray-300" / key={280431}>
          )}
      ...
      <div className="text-center" key={120206}>
        <div className="font-bold text-lg text-primary-600" key={555318}>{prop.player_name}</div>
        {gameInfo && (
      ...
      <div className="space-y-2 text-xs" key={270907}>
        ...
        <button; 
            onClick={(e) = key={252324}> handleExternalLink(e, `https://www.espn.com/search/results?q=${encodeURIComponent(prop.player_name)}`)}
