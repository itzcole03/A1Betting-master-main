     ...
     key={184538}>
      <div className="flex items-center justify-between mb-2" key={120997}>
        ...
        <div className="flex gap-4" key={764070}>
          <span className="text-xs font-semibold text-green-600 bg-green-100 px-2 py-1 rounded-full" key={586719}>
            +12.4% ROI;
          </span>
          <span className="text-xs font-semibold text-blue-600 bg-blue-100 px-2 py-1 rounded-full" key={553360}>
            68.4% Win Rate;
          </span>
        </div>
      </div>
      {(!Array.isArray(entries) || entries.length === 0) && (
        <p className="text-text-muted text-center mt-10" key={66411}>No betting history to display chart.</p>
      )}
