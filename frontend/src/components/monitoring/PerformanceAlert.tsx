...
const PerformanceAlert: React.FC<PerformanceAlertProps key={394969}> = ({ alert, onDismiss, onAcknowledge }) => {
  ...
  const getSeverityIcon = (severity: PerformanceAlert['severity']) => {
    switch (severity) {
      case 'critical':
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" key={687149}>
            <path;
              clipRule="evenodd"
    ...
    <motion.div;
     ...
     key={360280}>
      <div className="flex items-start" key={170970}>
        ...
        <div className="ml-3 w-full" key={835404}>
          <div className="flex justify-between items-start" key={678391}>
            <div key={241917}>
              <h3 className="text-sm font-medium" key={652613}>{alert.metric}</h3>
            ...
            <div className="flex space-x-2" key={753076}>
              {onAcknowledge && (
                <button;
                  ...
                  onClick={() = key={853147}> onAcknowledge(alert.id)}
                >
                  Acknowledge;
                </button>
              )}
              {onDismiss && (
                <button;
                  className="text-xs px-2 py-1 rounded bg-white bg-opacity-50 hover:bg-opacity-75 transition-colors"
                  onClick={() = key={853147}> onDismiss(alert.id)}
                >
                  Dismiss;
                </button>
              )}
