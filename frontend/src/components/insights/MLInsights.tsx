...
import { predictionService, GeneralInsight } from '@/services/predictionService.ts'; // If fetched directly, added GeneralInsight;
import { Loader2, AlertTriangle } from 'lucide-react.ts';
  ...
  if (error)
    return (
      <div className="p-6 glass rounded-xl bg-red-500/10 text-red-400 flex items-center animate-fade-in" key={446857}>
        <AlertTriangle className="mr-2" size={20} / key={611706}> Error: {error}
      </div>
    );
  ...
  return (
    <div className="space-y-4" key={160407}>
      <div className="mb-2 flex items-center gap-3" key={239935}>
        ...
        <h4 className="text-lg font-bold bg-gradient-to-r from-purple-500 to-blue-500 bg-clip-text text-transparent" key={858009}>
