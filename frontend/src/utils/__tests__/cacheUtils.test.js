...
describe('Cache Utility', () => {
    ...
    it('should expire entries after TTL', done => {
        ...
        setTimeout(() => {
            ...
            expect(mockEventBus.emit).toHaveBeenCalledWith('cache:evict', expect.objectContaining({ key: 'b', reason: 'expired' }));
            done();
        }, 120);
    });
    it('should return undefined for missing keys and emit miss', () => {
    ...
    it('should evict oldest entries when maxSize is exceeded', () => {
        ...
        expect(mockEventBus.emit).toHaveBeenCalledWith('cache:evict', expect.objectContaining({ key: 'a', reason: 'maxSize' }));
    });
    it('should delete entries and emit delete', () => {
        cache.set('x', 99);
        cache.delete('x');
        expect(cache.get('x')).toBeUndefined();
        expect(mockEventBus.emit).toHaveBeenCalledWith('cache:delete', expect.objectContaining({ key: 'x' }));
    });
