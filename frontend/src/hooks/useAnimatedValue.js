...
export function useAnimatedValue(initialValue, config = {}) {
    ...
    const animateSpring = useCallback(() => {
        const animate = () => {
            ...
            if (Math.abs(dx) < (spring.precision || 0.01) &&
    ...
    const animateLinear = useCallback((target, duration) => {
        ...
        const animate = (currentTime) => {
            ...
            setValue(startValueRef.current + (target - startValueRef.current) * progress);
            frameRef.current = requestAnimationFrame(animate);
        };
        setIsAnimating(true);
        frameRef.current = requestAnimationFrame(animate);
    }, [value, config, cancelAnimation]);
    const animateTo = useCallback((target, newConfig = {}) => {
        cancelAnimation();
        const mergedConfig = {
            ...config,
            ...newConfig
