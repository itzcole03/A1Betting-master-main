...
export const useQuantumBetting = (options) => {
    ...
    const calculateKellyStake = useCallback((odds, winProbability) => {
        return QuantumServices.betting.calculateKellyStake(bankroll, odds, winProbability);
    }, [bankroll]);
    const placeBet = useCallback(async (bet) => {
        ...
        try {
            ...
            QuantumServices.notifications.createAlert("error", "Bet placement failed");
            throw error;
        }
        finally {
            setLoading(false);
...
export const useQuantumAuth = () => {
    ...
    const logout = useCallback(() => {
        ...
        QuantumServices.notifications.createAlert("info", "Logged out successfully");
    }, []);
    return {
        user,
        loading,
...
export const useQuantumNotifications = () => {
    ...
    const createAlert = useCallback((type, message, data) => {
        QuantumServices.notifications.createAlert(type, message, data);
    }, []);
    ...
    const submit = useCallback(async (onSubmit) => {
        ...
        if (validate()) {
            try {
                ...
                QuantumServices.notifications.createAlert("success", "Form submitted successfully");
            }
            catch (error) {
                QuantumServices.notifications.createAlert("error", "Submission failed");
            }
        }
        setIsSubmitting(false);
    }, [values, validate]);
...
export const useQuantumUI = () => {
    ...
    const copyToClipboard = useCallback(async (text) => {
        try {
            ...
            QuantumServices.notifications.createAlert("success", "Copied to clipboard");
        }
        catch (error) {
