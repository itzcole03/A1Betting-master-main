...
class AppCompletenessAnalyzer:
        ...
        for file_path in backend_files:
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    if any(keyword in content for keyword in ['jwt', 'token', 'auth', 'login', 'user']):
                        has_auth_logic = True
                        break
            except:
                continue
    ...
    def analyze_api_endpoints(self) -> Dict[str, Any]:
        ...
        if not api_files:
            return {'status': 'missing', 'priority': 'critical', 'missing_files': ['backend/main.py']}
            ...
            try:
                with open(api_file, 'r', encoding='utf-8') as f:
                    ...
                    for endpoint in required_endpoints:
                        if endpoint in content or endpoint.replace('/api/', '/').replace('/auth/', '/') in content:
    ...
    def run_analysis(self) -> Dict[str, Any]:
        ...
        self.analysis_report['completion_status']['authentication'] = self.analyze_authentication_system()
        self.analysis_report['completion_status']['api_endpoints'] = self.analyze_api_endpoints()
        self.analysis_report['completion_status']['frontend_components'] = self.analyze_frontend_components()
        self.analysis_report['completion_status']['database_models'] = self.analyze_database_models()
    ...
    def _calculate_completion_percentage(self):
        ...
        completed_components = sum(1 for comp in self.analysis_report['completion_status'].values() 
...
if __name__ == "__main__":
    ...
    for component, status in report['completion_status'].items():
        ...
        print(f"{status_icon} {component.title().replace('_', ' ')}: {status['status']}")
    ...
    if report['recommendations']:
        ...
        for i, rec in enumerate(report['recommendations'], 1):
            ...
            print(f"      Priority: {rec['priority'].title()} | Est. Time: {rec['estimated_time']}")
