...
class DockerSecurityScanner:
    ...
    def run_security_scan(self) -> Dict[str, Any]:
        ...
        if compose_path.exists():
            results["scans"]["docker_compose"] = self.scan_docker_compose(str(compose_path))
        else:
            logger.warning("docker-compose.yml not found")
            results["scans"]["docker_compose"] = {
                "file": "docker-compose.yml",
                "issues": [{"issue": "docker-compose.yml not found", "severity": "INFO"}],
                "total_issues": 0
            }
...
def main():
    ...
    if results['summary']['total_issues'] > 0:
        high_severity_issues = sum(
            1 for scan in results['scans'].values()
            for issue in scan.get('issues', [])
            if issue.get('severity') in ['HIGH', 'CRITICAL']
        )
        if high_severity_issues > 0:
            logger.warning(f"Found {high_severity_issues} high/critical severity issues")
            sys.exit(1)
