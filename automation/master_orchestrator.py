...
class MasterOrchestrator:
        ...
        for step_config in config.get('steps', []):
            ...
            for task_config in step_config.get('tasks', []):
                task = Task(
                    ...
                    max_retries=task_config.get('max_retries', self.config.get('max_retries', 3)),
                    timeout=task_config.get('timeout', self.config.get('default_timeout', 300)),
    ...
    async def execute_workflow_step(self, step: WorkflowStep) -> bool:
        ...
        if step.parallel:
            ...
            if not step.continue_on_error and success_count < len(step.tasks):
                logger.error(f"Step failed: {step.name} - {len(step.tasks) - success_count} tasks failed")
                return False
        else:
    ...
    async def execute_workflow(self, workflow: Workflow) -> bool:
        ...
        try:
            ...
            logger.info(f"Workflow completed successfully: {workflow.name} (Duration: {duration:.2f}s)")
            return True
...
class VSCodeToolsIntegration:
    ...
    def _discover_tools(self) -> Dict[str, bool]:
        ...
        tools = {
            ...
            'typescript_extension': self._check_extension('ms-vscode.vscode-typescript-next'),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="A1Betting Master Automation Orchestrator")
    parser.add_argument('--config', default='automation/config.yaml', help='Configuration file path')
