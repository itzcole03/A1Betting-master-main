...
export function RealTimePredictions({
  ...
  // Use centralized sports config but also include any dynamic sports from predictions
  const dynamicSports = Array.from(new Set(predictions.map((p) => p.sport)));
  const allSportsSet = new Set([...SPORT_OPTIONS, ...dynamicSports]);
  const sports = Array.from(allSportsSet).filter(
    (sport) => sport === "All" || allSportsSet.has(sport),
  );
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-2">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-bold ${getValueGradeColor(prediction.valueGrade)}`}
                >
