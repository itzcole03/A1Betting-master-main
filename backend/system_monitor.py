        ...
        try:
            ...
            try:
                ...
                if hasattr(ultra_cache_optimizer, "cache"):
                    cache_stats = (
                        await ultra_cache_optimizer.cache.get_comprehensive_stats()
                    )
                    self.metrics_buffer.append(
                        SystemMetric(
            ...
            try:
                ...
                if hasattr(ultra_task_processor, "task_queue"):
                    queue_stats = (
                        await ultra_task_processor.task_queue.get_queue_stats()
                    )
                    self.metrics_buffer.append(
                        SystemMetric(
...
class AlertManager:
    ...
    async def create_alert(
        ...
        logger.warning(
            f"Alert created: {alert.title} (Severity: {alert.severity.name})"
        )
        return alert
